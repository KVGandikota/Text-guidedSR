...
def main (args):
    ...
    for restart in range(args.run):
            ...
            for i in range(args.count):
                ...
                #Within pipe stage 1
                image = stage_1.sup_res(lr = lrf.detach(),prompt_embeds=prompt_embeds, negative_prompt_embeds=negative_embeds, num_inference_steps=args.dec_steps, guidance_scale = args.guidance_scale_stage1, generator=generator, output_type="pt", sr_scale=scale//4)
                
                #pipe stage 2                
                image = stage_2.sup_res(image=image,lr = lrf.detach(),sr_scale=scale//4, prompt_embeds=prompt_embeds, negative_prompt_embeds=negative_embeds,  num_inference_steps=args.sr_steps, guidance_scale = args.guidance_scale_stage2, generator=generator, output_type="pt").images.clamp_(-1,1)
