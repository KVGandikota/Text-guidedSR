...
# Forward operators taken from Wang etal. Denoising diffusion Null-space Models and Kawar etal. Denoising Diffusion Restoration models
...
def MeanUpsample(x, scale):
        n, c, h, w = x.shape
        out = torch.zeros(n, c, h, scale, w, scale).to(torch.float16).to(x.device) + x.view(n,c,h,1,w,1)
        out = out.view(n, c, scale*h, scale*w)
        return out
  
    
def get_Afuncs(degrad,sizes,sr_scale=None,inpaintmask=None,blurkernel=None):
    if degrad == 'sr_bicubic':
        factor = int(sr_scale)
        ...
        k = k / np.sum(k)
        kernel = torch.from_numpy(k).to('cuda')
        A_funcs = SRConv(kernel / kernel.sum(), 3, sizes, 'cuda', stride=factor)
        return A_funcs      
    
    elif degrad == 'sr_average':
        A = torch.nn.AdaptiveAvgPool2d((sizes//int(sr_scale),sizes//int(sr_scale)))       
        Ap = lambda z: MeanUpsample(z,sr_scale)
        return A,Ap
